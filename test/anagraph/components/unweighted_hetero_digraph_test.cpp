...
TEST(HeteroDigraphTest, SetEdge) {
    using namespace anagraph;
    spdlog::set_level(spdlog::level::debug);
    ...
    EXPECT_FALSE(adjacents1.contains(0));
}
...
TEST(HeteroDigraphTest, ReadGraph) {
    ...
    EXPECT_FALSE(graph.getAdjacents(2).contains(0));
    EXPECT_FALSE(graph.getAdjacents(2).contains(1));
    EXPECT_FALSE(graph.getAdjacents(3).contains(2));
    EXPECT_FALSE(graph.getAdjacents(4).contains(2));
    EXPECT_FALSE(graph.getAdjacents(5).contains(4));
}
...
TEST(HeteroDigraphTest, WriteGraph) {
    ...
    const std::string outputPath = datasetDirectory + "/output/unweighted_hetero_digraph_output.txt";
    graph.writeGraph(outputPath, FileExtension::TXT);
}

TEST(HeteroDigraphTest, GraphIterator) {
